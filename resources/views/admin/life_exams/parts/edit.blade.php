        ...
        <div class="form-group">
            <div class="row">
                <div class="col-md-12 mt-3">
                    ...
                    <input type="text"  value="{{ $lifeExam->name_ar}}" class="form-control" name="name_ar">
                </div>
                <div class="col-md-12 mt-3">
                    <label for="name_en" class="form-control-label">اسم الامتحان باللغه الانجليزيه</label>
                    <input type="text" value="{{ $lifeExam->name_en}}" class="form-control" name="name_en">
                </div>
            </div>
            <div class="row">
                <div class="col-md-12 mt-3">
                    <label for="date_exam" class="form-control-label">تاريخ اداء الامتحان للطلبه*</label>
                    <input type="date"  value="{{ $lifeExam->date_exam}}" class="form-control" name="date_exam">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mt-3">
                    <label for="time_start" class="form-control-label">توقيت البدء للامتحان</label>
                    <input type="time"  value="{{ $lifeExam->time_start}}" class="form-control" name="time_start">
                </div>
                <div class="col-md-6 mt-3">
                    <label for="time_end" class="form-control-label">توقيت انتهاء الامتحان</label>
                    <input type="time"  value="{{ $lifeExam->time_end}}" class="form-control" name="time_end">
                </div>
            </div>
                    ...
                    <Select name="season_id" class="form-control">
                        ...
                        @foreach ($data['seasons'] as $season)
                            <option value="{{ $season->id }}"
                                    {{ $lifeExam->season_id == $season->id ? 'selected' : '' }}>{{ $season->name_ar }}</option>
                        @endforeach
                    </Select>
                </div>

                <div class="col-md-12 mt-3">
                    <label for="name_ar" class="form-control-label">اختر تيرم معين لهذا الصف*</label>
                    <Select name="term_id" class="form-control">
                        <option selected disabled>اختر تيرم معين لهذا الصف*</option>
                        @foreach ($data['terms'] as $term)
                            <option value="{{ $term->id }}"
                                    {{ $lifeExam->term_id == $term->id ? 'selected' : '' }}>
                                {{ $term->name_en }}</option>
                        @endforeach
                    </Select>
                </div>
            ...
            <div class="row">
                <div class="col-md-12 mt-3">
                    ...
                    <input type="number" value="{{ $lifeExam->quiz_minute}}" class="form-control" name="quiz_minute">
                </div>

                <div class="col-md-12 mt-3">
                    <label for="time_end" class="form-control-label">الدرجه الكليه لهذا الامتحان*</label>
                    <input type="number" value="{{ $lifeExam->degree}}" class="form-control" name="degree">
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
